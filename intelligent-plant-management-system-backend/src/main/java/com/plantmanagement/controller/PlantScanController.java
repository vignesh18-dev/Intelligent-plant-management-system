...
public class PlantScanController {
  ...
  public Map<String, String> scanPlant(@RequestParam("file") MultipartFile file) {
    ...
    try {
      ...
      String prompt = "You are FloraBot, an expert plant identification AI. " +
      ...
      if (root.has("choices") && root.get("choices").isArray() && root.get("choices").size() > 0) {
        ...
        String errorType = root.has("error") ? root.get("error").path("type").asText("unknown") : "unknown";
        String errorMsg  = root.has("error") ? root.get("error").path("message").asText("no message") : "no error field";
        log.error("Groq Vision API error — HTTP {}: type='{}', message='{}', full body: {}",
            response.statusCode(), errorType, errorMsg, response.body());
        answer = "AI image scan error: " + errorMsg;
      }
